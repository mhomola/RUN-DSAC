...
class VecOnlineRLAlgorithm(BaseRLAlgorithm, metaclass=abc.ABCMeta):
    ...
    def _log_stats(self, epoch):
        ...
        stats_eval, avg_returns = eval_util.get_generic_path_information(eval_paths)
        self.avg_returns.append(avg_returns)
        logger.record_dict(stats_eval, prefix="evaluation/")
        ...
        times   = _get_epoch_timings()
        wandb.log({"Average returns [-]": avg_returns, "Episode time [s]": times['time/epoch (s)'], 'Episode': epoch})
