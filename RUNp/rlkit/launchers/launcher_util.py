...
def setup_logger(exp_prefix         = "default",
    ...
    :param tabular_log_file:    Table for progress logging
    :param snapshot_mode:
...
def set_seed(seed):
    ...
    if seed is None:
        return                              # If seed is None, no need to set the random seeds
    ...
    if hasattr(gym.spaces, "prng"):         # Check if gym.spaces has the attribute "prng"
...
def run_experiment(
    ...
    if use_gpu and mode == 'ec2':
        ...
        if region == 'us-east-1':
            ...
            mode_kwargs['extra_ec2_instance_kwargs'] = dict(Placement=dict(AvailabilityZone=avail_zone,),)
    else:
        image_id = None
...
def create_mounts(
    ...
    if mode == 'ec2':
        output_mount = mount.MountS3(
            ...
            output=True,
            sync_interval=sync_interval,
            include_types=('*.txt', '*.csv', '*.json', '*.gz', '*.tar', '*.log', '*.pkl', '*.mp4', '*.png', '*.jpg',
                           '*.jpeg', '*.patch'),
        )
    elif mode == 'gcp':
